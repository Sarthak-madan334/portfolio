...
const navItems = ["About", "Stack", "Projects", "Open Source", "Contact"];
...
export function Navbar() {
  ...
  const toId = (label: string) => `#${label.toLowerCase().replace(" ", "-")}`;
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-3 z-50 px-3 transition-all duration-300">
      <nav className={`liquid-glass pointer-events-auto mx-auto flex h-[66px] w-full max-w-[1180px] items-center justify-between rounded-[24px] px-4 sm:px-6 ${scrolled ? "liquid-glass-scrolled" : ""}`} aria-label="Main navigation">
        ...
        <div className="hidden items-center gap-7 lg:flex">{navItems.map((item) => <a key={item} href={toId(item)} className="focus-ring rounded-full px-1 text-xs font-medium text-[#55555a] transition hover:text-black">{item}</a>)}</div>
      ...
      <AnimatePresence>
        {open && <motion.div initial={{ opacity: 0, height: 0, y: -8 }} animate={{ opacity: 1, height: "auto", y: 0 }} exit={{ opacity: 0, height: 0, y: -8 }} className="liquid-glass pointer-events-auto mx-auto mt-2 w-[calc(100%_-_24px)] max-w-[1180px] overflow-hidden rounded-[24px] lg:hidden"><div className="flex flex-col px-5 py-4">{navItems.map((item) => <a key={item} href={toId(item)} onClick={() => setOpen(false)} className="flex items-center justify-between border-b border-black/[0.06] py-4 text-sm font-medium text-[#4c4c50]">{item}<ArrowUpRight size={14} /></a>)}<a href="/Sarthak-Madan-Resume.pdf" download className="mt-4 flex items-center justify-center gap-2 rounded-full bg-[#1d1d1f] px-4 py-3 text-sm font-semibold text-white"><Download size={15} /> Download resume</a></div></motion.div>}
      </AnimatePresence>
    </header>
  );
}
